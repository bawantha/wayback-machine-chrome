...
function remove_alexa(url) {
    const pos = url.indexOf('/siteinfo/');
    const new_url = url.substring(pos + 10);
    return remove_port(new_url);
}

function remove_whois(url) {
    const pos = url.indexOf('/whois/');
    const new_url = url.substring(pos + 7);
    return remove_port(new_url);
}
...
function social_share(eventObj) {
    const parent = eventObj.target.parentNode;
...
function alexa_statistics(eventObj) {
    const open_url = `http://www.alexa.com/siteinfo/${get_clean_url()}`;
    window.open(open_url, 'newwindow', 'width=1000, height=1000,left=0');
}

function whois_statistics(eventObj) {
    const open_url = `https://www.whois.com/whois/${get_clean_url()}`;
    window.open(open_url, 'newwindow', 'width=1000, height=1000,left=0');
}
...
function search_tweet(eventObj) {
    ...
    const open_url = `https://twitter.com/search?q=${url}`;
    window.open(open_url, 'newwindow', 'width=1000, height=1000,left=0');
}
...
function display_suggestions(e) {
    ...
    window.setTimeout(() => {
        const len = document.getElementById('search_input').value.length;
        if ((len) >= 3) {
